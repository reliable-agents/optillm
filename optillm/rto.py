...
def round_trip_optimization(system_prompt: str, initial_query: str, client, model: str) -> str:
    ...
    response_c1 = client.chat.completions.create(model=model, messages=messages, max_tokens=4096, n=1, temperature=0.1)
    ...
    messages.append(
        {
            "role": "user",
            "content": "Summarize or describe the code you just created. The summary should be in form of an instruction such that, given the instruction you can create the code yourself.",
        }
    )
    response_q2 = client.chat.completions.create(model=model, messages=messages, max_tokens=1024, n=1, temperature=0.1)
    ...
    response_c2 = client.chat.completions.create(model=model, messages=messages, max_tokens=4096, n=1, temperature=0.1)
    ...
    response_c3 = client.chat.completions.create(model=model, messages=messages, max_tokens=4096, n=1, temperature=0.1)
