...
if os.environ.get("OPENAI_API_KEY"):
    ...
    if API_KEY is not None:
        ...
        default_client = AzureOpenAI(
            api_version=API_VERSION, azure_endpoint=AZURE_ENDPOINT, azure_ad_token_provider=token_provider
        )
else:
    ...
    if approach in known_approaches:
        ...
        if approach == "mcts":
            ...
            rstar = RStar(
                ...
                client,
                model,
                max_depth=server_config["rstar_max_depth"],
                num_rollouts=server_config["rstar_num_rollouts"],
                c=server_config["rstar_c"],
            )
            return rstar.solve(initial_query)
        elif approach == "cot_reflection":
            return cot_reflection(
                system_prompt, initial_query, client, model, return_full_response=server_config["return_full_response"]
            )
...
def check_api_key():
    if server_config["optillm_api_key"]:
        ...
        if not auth_header or not auth_header.startswith("Bearer "):
            return jsonify(
                {"error": "Invalid Authorization header. Expected format: 'Authorization: Bearer YOUR_API_KEY'"}
            ), 401
...
def proxy():
    ...
    try:
        if operation == "SINGLE":
            final_response, completion_tokens = execute_single_approach(
                approaches[0], system_prompt, initial_query, client, model
            )
        elif operation == "AND":
            final_response, completion_tokens = execute_combined_approaches(
                approaches, system_prompt, initial_query, client, model
            )
        elif operation == "OR":
            loop = asyncio.new_event_loop()
            asyncio.set_event_loop(loop)
            final_response, completion_tokens = loop.run_until_complete(
