...
class MockOpenAIClient:
    def chat_completions_create(self, *args, **kwargs):
        class MockResponse:
            def __init__(self):
                self.choices = [
                    type("obj", (object,), {"message": type("obj", (object,), {"content": "Mock response"})()})
                ]
...
def run_test_case(test_case: Dict, approaches: List[str], client, model: str) -> Dict:
    ...
    with ThreadPoolExecutor() as executor:
        future_to_approach = {
            executor.submit(run_approach, approach, system_prompt, query, client, model): approach
...
def main():
    ...
    parser.add_argument(
        "--base-url", type=str, default=None, help="The base_url for the OpenAI API compatible endpoint"
    )
